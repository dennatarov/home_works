...
# def solution_t2_01(N):
    ...
    #     pip = n_bin.find('1', 1)
    #     while pip != -1:
    #         n_length = pip - n_bin.rfind('1', 0, pip) - 1
    #         if n_length > gap_length: gap_length = n_length
    #         pip = n_bin.find('1', pip+1)
    #     return gap_length

def solution_t2_02(N):
    if N is 0:
        return 0
    else:
        counter = []
        for elem in bin(N)[2:bin(N).rfind('1')+1]:
            if elem is '1':
                p = 0   # p is defined here, because binary number has first digit 1 (if N is not 0)
                counter.append(p)
            else:
                p += 1
                counter.append(p)
